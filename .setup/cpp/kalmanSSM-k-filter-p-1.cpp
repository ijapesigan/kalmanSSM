...
Rcpp::List KFilterP1(const arma::mat& data, const arma::mat& Lambda,
  ...
  int k = data.n_cols;
  int j = beta.n_rows;
  arma::cube eta_predicted(j, 1, time);
  arma::cube eta_cov_predicted(j, j, time);
  arma::cube eta_filtered(j, 1, time);
  arma::cube eta_cov_filtered(j, j, time);
  arma::cube innovations(k, 1, time);
  arma::cube sigma_innovations(k, k, time);

  // Step 2: Initialize state variables at time 0
  eta_predicted.slice(0) = beta * mu0;
  eta_cov_predicted.slice(0) = beta * Sigma0 * beta.t() + Psi;

  // Step 3: Compute initial innovation statistics
  arma::mat sigma_innovations_temp =
      Lambda * eta_cov_predicted.slice(0) * Lambda.t() + Theta;
  ...
  arma::mat gain =
      eta_cov_predicted.slice(0) * Lambda.t() * sigma_innovations_inv;
  innovations.slice(0) = data.row(0).t() - Lambda * eta_predicted.slice(0);
  eta_filtered.slice(0) = eta_predicted.slice(0) + gain * innovations.slice(0);
  eta_cov_filtered.slice(0) =
      eta_cov_predicted.slice(0) - gain * Lambda * eta_cov_predicted.slice(0);
  ...
  for (int i = 1; i < time; ++i) {
    // Predict step
    eta_predicted.slice(i) = beta * eta_filtered.slice(i - 1);
    eta_cov_predicted.slice(i) =
        beta * eta_cov_filtered.slice(i - 1) * beta.t() + Psi;

    // Compute innovation statistics
    sigma_innovations_temp =
        Lambda * eta_cov_predicted.slice(i) * Lambda.t() + Theta;
    sigma_innovations.slice(i) =
        (sigma_innovations_temp.t() + sigma_innovations_temp) / 2.0;
    sigma_innovations_inv = arma::inv(sigma_innovations.slice(i));
    gain = eta_cov_predicted.slice(i) * Lambda.t() * sigma_innovations_inv;
  ...
  return Rcpp::List::create(
      Rcpp::Named("eta_predicted") = eta_predicted,
      Rcpp::Named("eta_cov_predicted") = eta_cov_predicted,
      Rcpp::Named("eta_filtered") = eta_filtered,
      Rcpp::Named("eta_cov_filtered") = eta_cov_filtered,
